...
// Navigation Waypoint (Fixed: Ensure `coordinate` is used instead of `location`)
typedef struct {
    geo_location_t coordinate;  // Waypoint location
    float heading;                // Desired heading at waypoint
    float loiter_time;            // Time to wait at waypoint (seconds)
    float acceptance_radius;      // Radius within which waypoint is considered reached
    bool is_current;              // Is this the current active waypoint?
} waypoint_t;
