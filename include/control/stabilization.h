...
// extern motor_context_t motor_ctx;
...
#include "drivers/mpu6050.h"
#include "drivers/radio_receiver.h"  // Ensure this contains receiver_input_t
#include "drivers/motor_control.h"   // Ensure this contains motor_outputs_t
#include "control/pid.h"
#include "types.h"
...
typedef struct {
    stabilization_mode_t mode;
    float max_angle_setpoint;    // Max angle setpoint (degrees)
    float max_rate_setpoint;     // Max rate setpoint (degrees/sec)
    float acro_rate_multiplier;  // Acro mode rate multiplier
    float horizon_mode_strength; // Horizon mode mixing strength
} stabilization_config_t;
